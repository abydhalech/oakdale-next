...
export default function QuickQuote() {
  ...
  function updateVia(i: number, val: string) {
    ...
    >
      ...
      {!submitted ? (
        <form
          ...
          autoComplete="on"
        >
          {/* Row 1: pickup / depart date / depart time */}
          ...
          {/* Row 2: destination / return date / return time */}
          <div className="grid md:grid-cols-3 gap-4">
            <input
              className="border border-black/20 rounded-lg px-4 h-12 focus:outline-none focus:ring-2 focus:ring-black"
              name="destination"
          ...
          <input
            type="hidden"
            name="_subject"
            value="Quick quote — Oakdale Travel"
          />
          ...
          {!FORMSPREE_ENDPOINT && (
            <p className="text-amber-600 text-sm mt-2">
              ...
              <code>NEXT_PUBLIC_FORMSPREE_ENDPOINT</code> in Vercel.
            </p>
          )}
        </form>
      ) : (
